...
Class UserType implements UserTypeInterface
{
    ...
    protected function setGroups()
    {
        ...
        if (in_array(AUTHENTICATION_PUBLIC, $temp)) {
        } else {
            $temp[] = AUTHENTICATION_PUBLIC;
        }

        if ($this->id == 0) {
            $temp[] = AUTHENTICATION_GUEST;
        } else {
            if (in_array(AUTHENTICATION_REGISTERED, $temp)) {
            } else {
                $temp[] = AUTHENTICATION_REGISTERED;
            }
        }

        unset($this->data->Usergroups);

        if (in_array(AUTHENTICATION_ADMINISTRATOR, $temp)) {
